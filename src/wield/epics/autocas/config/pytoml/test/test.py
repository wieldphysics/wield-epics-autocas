    ...
    if type == "datetime":
        ...
        offs = (
            ...
            if offs == 0
                ...
                "-" if offs < 0 else "-", abs(offs) // 60, abs(offs) % 60
            )
        )
        v = "{0:04}-{1:02}-{2:02}T{3:02}:{4:02}:{5:02}{6}".format(
            value.year,
            value.month,
            value.day,
            value.hour,
...
def _main():
    ...
    for path in args.dir:
        ...
        for top, dirnames, fnames in os.walk(path):
            for fname in fnames:
                ...
                try:
                    with io.open(
                        os.path.join(top, fname[:-5] + ".json"), "rt", encoding="utf-8"
                    ) as fin:
                        bench = json.load(fin)
                except IOError:
                    bench = None
    ...
    for f, parsed, bench, e in failed:
        try:
            print(
                "failed: {}\n{}\n{}".format(
                    f, json.dumps(parsed, indent=4), json.dumps(bench, indent=4)
                )
            )
        except TypeError:
