#! /usr/bin/env python
# Take a transfer function with the HP4195A Network Analyzer
#
#Eric Oelker  6/20/2012
...
parser.add_option("-r",
                  dest="atr", type = "str", default="10DB",
                  help="Reference input Attenuation.  Specify in DB.")

parser.add_option("-t",
                  dest="att", type="str", default="10DB",
...
gpibObj.command("OSC"+str(options.chan)+"="+str(options.pwr)) #Set the source power
time.sleep(0.1)
gpibObj.command("ATR"+str(options.chan)+"="+str(options.atr)) #Set Reference Input Attenuation
time.sleep(0.1)
gpibObj.command("ATT"+str(options.chan)+"="+str(options.att)) #Set Test Input Attenuation
time.sleep(0.1)
