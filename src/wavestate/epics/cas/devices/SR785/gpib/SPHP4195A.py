#! /usr/bin/env python
#This script automates a spectrum measurement with an HP4195A analyzer.
...
parser.add_option("-f", "--file", dest="filename",
                  help="Output file name without an extension", default="/data/eoelker/test")
...
parser.add_option("--R1",
                  dest="r1", default=False,
                  action="store_true",
                  help="Measure Input R1")

parser.add_option("--T1",
                  dest="t1", default=False,
                  action="store_true",
                  help="Measure input T1")
                  
parser.add_option("--R2",
                  dest="r2", default=False,
                  action="store_true",
                  help="Measure input R2")

parser.add_option("--T2",
                  dest="t2", default=False,
                  action="store_true",
                  help="Measure input T2")
parser.add_option("--AR1",
                  dest="ar1", type="str", default="10DBM",                  
                  help="Attenuation for input R1")

parser.add_option("--AT1",
                  dest="at1", type="str", default="10DBM",
                  help="Attenuation for input T1")
                  
parser.add_option("--AR2",
                  dest="ar2", type="str", default="10DBM",
                  help="Attenuation for input R2")

parser.add_option("--AT2",
                  dest="at2", type="str", default="10DBM",
                  help="Attenuation for input T2")
...
HP4195A.getdata(gpibObj,dataFile,2)  #Downloads data. Last parameter=2 indicates spectrum analyzer mode
time.sleep(0.1)
...
if options.plotData: 
    import gpibplot
    gpibplot.plotSPHP4195A(options.filename,options.title,xlog=options.xlog,ylog=options.ylog,psdunits=options.units)
