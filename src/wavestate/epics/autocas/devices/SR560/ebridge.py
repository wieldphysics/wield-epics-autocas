...
class SR560Ebridge(epics.EpicsRelayUser):
    ...
    def augment_medm(self, pvs_by_part, extra_replace, prefix = ''):
        super(SR560Ebridge, self).augment_medm(pvs_by_part, extra_replace, prefix = prefix)
    ...
    def rv_low_pass_knee(self):
        rv = declarative.RelayValue(0, validator = declarative.min_max_validator_int(0, 15))
        return rv

    @RELAY_EPICS.float_value_add('HIGH_PASS_KNEE_N', precision = 0, value = 0)
    @declarative.dproperty
    def rv_high_pass_knee(self):
        rv = declarative.RelayValue(0, validator = declarative.min_max_validator_int(0, 11))
        return rv

    @RELAY_EPICS.float_value_add('VERNIER_GAIN', precision = 0, value = 0)
    @declarative.dproperty
    def rv_vernier_gain(self):
        rv = declarative.RelayValue(0, validator = declarative.min_max_validator_int(0, 100))
        return rv

    @RELAY_EPICS.float_value_add('USEVERNIER', precision = 0, value = 0)
    @declarative.dproperty
    def rv_use_vernier(self):
        rv = declarative.RelayValue(0, validator = declarative.min_max_validator_int(0, 1))
        return rv
    ...
    @declarative.dproperty
    def rv_filter(self):
        rv = declarative.RelayValue(0, validator = declarative.min_max_validator_int(0, 5))
        return rv

    @RELAY_EPICS.float_value_add('INPUT', precision = 0, value = 0)
    @declarative.dproperty
    def rv_input(self):
        rv = declarative.RelayValue(0, validator = declarative.min_max_validator_int(0, 2))
        return rv
    ...
    @declarative.dproperty
    def rv_gain(self):
        rv = declarative.RelayValue(0, validator = declarative.min_max_validator_int(0, 14))
        return rv
