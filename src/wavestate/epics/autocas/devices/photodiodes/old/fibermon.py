...
class FiberMonEBridge(
    ...
    @RELAY_EPICS.float_value_add('OUTP_PERCTOT', precision = 2, value = 0, binding_type = 'ext')
    @declarative.dproperty
    def rv_outP_percent_total(self):
        rv = declarative.RelayValue(0)
        return rv

    @RELAY_EPICS.float_value_add('OUTS_PERCTOT', precision = 2, value = 0, binding_type = 'ext')
    @declarative.dproperty
    ...
    def augment_medm(self, pvs_by_part, extra_replace, prefix = ''):
        super(FiberMonEBridge, self).augment_medm(pvs_by_part, extra_replace, prefix = prefix)
...
class FiberMonController(
    ...
    def dividers_setup(self):
        def div_all(value = None):
            self.ebridge.rv_out_total.value = self.ebridge.outS.rv_mw.value + self.ebridge.outP.rv_mw.value
            try:
                self.ebridge.rv_out_total_percent.value = 100 * self.ebridge.rv_out_total.value / self.ebridge.inmon.rv_mw.value
            except ZeroDivisionError:
                self.ebridge.rv_out_total_percent.value = -8888.8888
                pass
            try:
                self.ebridge.rv_outP_percent_total.value = 100 * (self.ebridge.outP.rv_mw.value / self.ebridge.rv_out_total.value)
            except ZeroDivisionError:
                self.ebridge.rv_outP_percent_total.value = -8888.8888
                pass
            try:
                self.ebridge.rv_outS_percent_total.value = 100 * (self.ebridge.outS.rv_mw.value / self.ebridge.rv_out_total.value)
            except ZeroDivisionError:
