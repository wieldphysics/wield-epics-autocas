    ...
    if type == 'spectrum':
        for line in fileinput.input(dataFile):
            ...
            if firstLine:
                ...
                data[dispID]=hstack((data[dispID], transpose(array([map(float,line.split())]))))
...
def plotTSSR785(filename):
    ...
    for line in fileinput.input(dataFile):
        ...
        if firstLine:
            ...
            if timeSeries:
                tsData[tDispID]=hstack((tsData[tDispID], transpose(array([map(float,line.split())]))))
            else:
                hsData[hDispID]=hstack((hsData[hDispID], transpose(array([map(float,line.split())]))))
