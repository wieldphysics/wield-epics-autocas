...
def parse_commandline():
    ...
    parser.add_option(
        "-p",
        "--TFSR785",
        default="./TFCoherSR785.py",
        help="Path to the TFCoherSR785.py.",
    )
    parser.add_option(
        "-i",
        "--ip",
        dest="ipAddress",
        default="gpib9.mit.edu",
...
if opts.inputFile != None:
    ...
    if opts.skipReset:
        ...
        reset = ""
    cmd = (
        'python %s -i %s -a %s %s --coherence -f %s -s %s -e %s -n %s --sweep %s -x %s -c %s -t %s --ic1 %s --ic2 %s --ig1 %s --ig2 %s --inputrange1 %s --inputrange2 %s --armode %s --title "%s" --memo "%s"'
        % (
            opts.TFSR785,
            opts.ipAddress,
            opts.gpibAddress,
