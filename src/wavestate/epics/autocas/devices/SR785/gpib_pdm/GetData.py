...
# Modified by Aaron Buikema on 10/17/2013
...
def getdata(gpibObj, dataFile, paramFile):
    ...
    isSpectra = not int(
        gpibObj.query("MGRP?" + str(disp), 128)
    )  # True if it is FFT measurement

    if isSpectra:  # If FFT group
        for disp in range(numOfDisp):
            dataFile.write(
                "#Display #" + str(disp + 1) + " length= " + str(len(freq[disp])) + "\n"
            )
            unit = gpibObj.query("UNIT?%d" % disp)
            dataFile.write("# Freqency(Hz) %s\n" % unit[:-1])
...
def downloadData(gpibObj, disp):
    ...
    numPoint = int(gpibObj.query("DSPN?" + str(disp), 128))
    ...
    for b in range(numPoint):  # Loop for frequency bins
        percent = int(math.floor(100 * b / numPoint))
        if (percent - accomplished) >= 1 and percent < 100:
            print "\010" + "\010" * txtlen + "%d%%" % percent,
            sys.stdout.flush()
            txtlen = len(str(percent)) + 1
            accomplished = percent
            pass

        f = gpibObj.query("DBIN?" + str(disp) + "," + str(b), IFCCheck=False)
        f = f[:-1]  # Chop new line character
        d = gpibObj.query("DSPY?" + str(disp) + "," + str(b), IFCCheck=False)
...
ip = "18.2.120.187"
address = 10
gpibObj = netgpib.netGPIB(ip, address, eot="\004")
gpibObj.connect()

# run TFSR785.py to get data
title = time.strftime("%Y-%m-%d.%H%M%S", time.gmtime())
dtitle = title + "d"
ptitle = title + "p"
dataName = os.path.join(dtitle)
dataFileName = dataName + ".dat"
paramName = os.path.join(ptitle)
paramFileName = paramName + ".dat"
...
filename = title
getparam(gpibObj, filename, dataFile, paramFile)
