...
filetime = time.strftime("%Y-%m-%d.%H%M%S",time.gmtime()) # Date string for data file (default

parser = optparse.OptionParser(usage=usage)
parser.add_option("-f", "--file", dest="filename",
                  help="Output file name without an extension", default=filetime)
parser.add_option("-i", "--ip",
                  dest="ipAddress", default="gpib5.mit.edu",
...
parser.add_option( "--coherence", dest="Coherence", action="store_true", 
		  default=False,help="If given, the code stores Normalised Variance of channel 2 as column 4.")
...
parser.add_option("-n", "--numpoints",
                  dest="numOfPoints", type="int", default="200",
...
## input sanity check
sf=options.startFreq.replace('Hz','').replace('k','E3').replace('m','E-3').replace('u','E-6')
try:
  float(sf)
except ValueError:
...
ef=options.stopFreq.replace('Hz','').replace('k','E3').replace('m','E-3').replace('u','E-6')
try:
  float(ef)
except ValueError:
...
gpibObj.command('MEAS0,47') # Frequency Resp 
...
gpibObj.command('MEAS1,47') # Frequency Resp 
...
if options.Coherence :
 print('Changing Display 1 to Norm Variance 2')
 ##Get Coherence data :
 ## CHange display 0
 gpibObj.command('MEAS0,45') # Normalized Variance 2
 ...
 i=int(gpibObj.query("MEAS?"+str(disp)))
 if i == 45:
    measurement.append('Normalized variance 2')
 else:
    print >> sys.stderr, "Error: Measurement is not Normalized Variance."
    sys.exit(1)
 i=int(gpibObj.query("VIEW?"+str(disp)))
 view.append({0: 'Log Magnitude', 1: 'Linear Magnitude',
              2: 'Magnitude Squared', 3: 'Real Part', 4: 'Imaginary Part',
...
if options.sweepType == "Log":
  if startF < endF:
    freq=[10**((log10(endF)-log10(startF))*i/(numPoint-2)+log10(startF)) for i in range(numPoint)]
  else:
    freq=[10**((log10(startF)-log10(endF))*i/(numPoint-2)+log10(endF)) for i in range(numPoint)]
else: # linear
  if startF > endF:
...
if options.Coherence :
  #dataFile.write('## Freqency(Hz) Magnitude(%s) Phase(%s) Coherence\n'%(units[0],units[1],units[2]))
  dataFile.write('## Freqency(Hz) Magnitude(%s) Phase(%s) Coherence\n'%(units[0],units[1]))
