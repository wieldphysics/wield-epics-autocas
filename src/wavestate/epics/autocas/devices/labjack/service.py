...
class LJRelay(cas9epics.OverridableObject):
    ...
    def _state_enable_do(self, bval):
        if bval:
            ...
            self.reactor.send_task(lambda : self._connect_start(self._connection_token, first_try = True))
        else:
            self._connection_token += 1
            self.reactor.send_task(lambda : self._connect_close(self._connection_token, finished = True))
            return
    ...
    def rv_sample_Hz(self):
        rv = cas9epics.RelayValue(2, cas9epics.min_max_validator(.01, 128))
        return rv
