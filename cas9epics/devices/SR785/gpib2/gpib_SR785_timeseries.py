#! /usr/bin/env python2
#
# TSSR785.py [-f filename] [-i ip_address] [-a gpib_address] [{-b|--bandwidth} bandwidth]
...
parser.add_option("-t", "--time",
                  dest="time", default=1.0,type="float",
                  help="Measurement time in sec.")
parser.add_option("-n", "--numbin",
                  dest="nbin", type="int", default=256,
                  help="Number of bins for histograms")
parser.add_option("-l", "--hlen",
                  dest="hlen", type="string", default="1",
...
for disp in range(numDisp):
    gpibObj.command('ACTD'+str(disp)) # Change active display
    time.sleep(0.1)
    gpibObj.command('MEAS'+str(disp)+','+str(72+disp)) # 72:Histogram 1, 73:Histogram 2
    time.sleep(0.1)
    print('Downloading data from display #'+str(disp))
