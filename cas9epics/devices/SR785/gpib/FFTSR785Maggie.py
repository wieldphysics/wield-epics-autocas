...
for disp in range(numDisp):
    ...
    # somehow the options parser never sets DB mode on, so options.decible is always false, so the below are the actual options used:
	print "setting up display in PSD mode"
        gpibObj.command('UNDB'+str(disp)+',1') # dB ON = 1, dB OFF = 0
        gpibObj.command('UNPK'+str(disp)+',2') # 1 = Vpk, 2 = Vrms, 3 = Vpp
