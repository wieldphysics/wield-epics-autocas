...
def parse_commandline():
  ...
  parser.add_option("-o", "--outputDir",default="outputs",help="Output directory name.")
  parser.add_option("--title",dest="title", type="string",default="",
                    help="Title of the measurement.")
  parser.add_option("--showplot",dest="showPlot", action="store_true",
		    default=False,help="If given, plot will show up on screen. (Default: False)")
  parser.add_option("-f", "--inputFile",help="If --inputFile is specified, the program just plots the data in the file. The file is assumed to be an output file from FFTSR785.py. If this option is not given, then the program runs the FFTSR785.py to get data, using the options given below.")
  

  # options for FFTSR785.py
  parser.add_option("-p", "--FFTSR785",default="./FFTSR785.py",help="Path to the FFTSR785.py.")
  ...
  parser.add_option("-i", "--ip",
                  dest="ipAddress", default="gpib9.mit.edu",
  ...
  parser.add_option("-r", "--skipreset", dest="skipReset", action="store_true", 
		  default=False,help="If given, the code does not reset the instrument at the beginning.")
  ...
  parser.add_option("-v", "--averaging",
                  dest="numAvg", type="int",default="50",
  ...
  parser.add_option("--timerecordincr",
                  dest="timeRecordIncr", default='""',
  ...
  parser.add_option("--ic1",
                  dest="inputCoupling1", type="string",default="AC",
                  help="CH1 input coupling. DC or AC")
  parser.add_option("--ic2",
                  dest="inputCoupling2", type="string",default="AC",
  ...
  parser.add_option("--inputrange1",
                  dest="inputRange1",default='""',
                  help="CH1 input range (default unit is dBVpk). If not given, auto-range will be used.")
  parser.add_option("--inputrange2",
                  dest="inputRange2",default='""',
                  help="CH2 input range (default unit is dBVpk). If not given, auto-range will be used.")
  parser.add_option("--id1",
                  dest="inputDiff1", type="string",default="SE",
                  help="CH1 input Differential A-B, or single ended A")
  parser.add_option("--id2",
                  dest="inputDiff2", type="string",default="SE",
                  help="CH1 input Differential A-B, or single ended A")
...
if opts.inputFile != None:
  ...
  if opts.skipReset:
    ...
    reset=""
  cmd = 'python %s -i %s -a %s -f %s %s -b %s -n %s -v %s --avgmode %s --timerecordincr %s %s --ic1 %s --ic2 %s --ig1 %s --ig2 %s --inputrange1 %s --inputrange2 %s --id1 %s --id2 %s -w %s --title "%s" --memo "%s"'%(opts.FFTSR785,opts.ipAddress,opts.gpibAddress,fileNameHigh,reset,opts.bandWidth,opts.numOfPoints,opts.numAvg,opts.avgMode,opts.timeRecordIncr,dual,opts.inputCoupling1,opts.inputCoupling2,opts.inputGND1,opts.inputGND2,opts.inputRange1,opts.inputRange2,opts.inputDiff1,opts.inputDiff2,opts.windowFunc,opts.title,opts.memo)
  if opts.verbose:
  ...
  if not opts.nocombine: 
    ...
    #cmd = 'python %s -i %s -a %s -f %s %s -b %s -n %s -v %s --avgmode %s --timerecordincr %s %s --ic1 %s --ic2 %s --ig1 %s --ig2 %s --inputrange1 %s --inputrange2 %s --id1 %s --id2 %s -w %s --title "%s" --memo "%s"'%(opts.FFTSR785,opts.ipAddress,opts.gpibAddress,fileNameLow,reset,'1.6kHz',opts.numOfPoints,opts.numAvg,opts.avgMode,opts.timeRecordIncr,dual,opts.inputCoupling1,opts.inputCoupling2,opts.inputGND1,opts.inputGND2,opts.inputRange1,opts.inputRange2,opts.inputDiff1,opts.inputDiff2,opts.windowFunc,opts.title,opts.memo)
    cmd = 'python %s -i %s -a %s -f %s -r -b %s -n %s -v %s --avgmode %s --timerecordincr %s %s --ic1 %s --ic2 %s --ig1 %s --ig2 %s --inputrange1 %s --inputrange2 %s --id1 %s --id2 %s -w %s --title "%s" --memo "%s"'%(opts.FFTSR785,opts.ipAddress,opts.gpibAddress,fileNameLow,'1.6kHz',opts.numOfPoints,opts.numAvg,opts.avgMode,opts.timeRecordIncr,dual,opts.inputCoupling1,opts.inputCoupling2,opts.inputGND1,opts.inputGND2,opts.inputRange1,opts.inputRange2,opts.inputDiff1,opts.inputDiff2,opts.windowFunc,opts.title,opts.memo)
    if opts.verbose:
...
if numOfDisp == 2:
  ...
  #plt.ylabel(units[1])
  #plt.grid()
  #plt.savefig(os.path.join(opts.outputDir,"%s_disp1.png"%title))
  #if opts.showPlot:
  #  print "Close the plot window to continue..."
  #  plt.show()
  #plt.close('all')
  ...
  #plt.ylabel(units[2])
  #plt.grid()
  #plt.savefig(os.path.join(opts.outputDir,"%s_disp2.png"%title))
  #if opts.showPlot:
  #  print "Close the plot window to continue..."
  #  plt.show()
  #plt.close('all')
  ...
  plt.grid()
  plt.legend(loc='best')
  plt.savefig(os.path.join(opts.outputDir,"%s_both.png"%title))
  if opts.showPlot:
    print "Close the plot window to continue..."
    plt.show()
  plt.close('all')
  ...
  #plt.grid()
  #plt.savefig(os.path.join(opts.outputDir,"%s_subplot.png"%title))
  #if opts.showPlot:
  #  print "Close the plot window to continue..."
  #  plt.show()
  #plt.close('all')
