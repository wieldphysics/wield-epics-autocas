...
# Modified by Aaron Buikema (added option to measure and plot coherence) 16/6/14
# and use with FFT measurement group 24/6/14
#
...
def parse_commandline():
  ...
  parser.add_option("-f", "--inputFile",help="If input file is specified, the program just plots the data in the file. The file is assumed to be an output file from TFSR785.py. If this option is not given, then the program runs the TFSR785.py to get data, using the options given below.")


  # options for TFCohereSR785_FFT.py
#   parser.add_option("-p", "--TFSR785",default="./TFSR785.py",help="Path to the TFSR785.py.")
  parser.add_option("-p", "--TFSR785",default="./TFCohereSR785_FFT.py",help="Path to the TFCohereSR785_FFT.py.")
  parser.add_option("-i", "--ip",
                  dest="ipAddress", default="gpib7.mit.edu",
  ...
  parser.add_option( "--coherence", dest="Coherence", action="store_true", 
		  default=True,help="If given, the code stores Coherence as column 4. Cannot turn off now.")
...
if opts.inputFile != None:
  ...
  if opts.skipReset:
    ...
    reset=""
  cmd = 'python %s -i %s -a %s %s -v %s --coherence -b %s -f %s --tsource %s --level %s -n %s --del1 %s --del2 %s --timerecordincr %s -w %s --ic1 %s --ic2 %s --ig1 %s --ig2 %s --inputrange1 %s --inputrange2 %s --title "%s" --memo "%s"'%(opts.TFSR785,opts.ipAddress,opts.gpibAddress,reset,opts.numAvg,opts.bandWidth,fileName,opts.triggerSource,opts.trig_level,opts.numOfPoints,opts.delayA,opts.delayB,opts.timeRecordIncr,opts.windowFunc,opts.inputCoupling1,opts.inputCoupling2,opts.inputGND1,opts.inputGND2,opts.inputRange1,opts.inputRange2,opts.title,opts.memo) #Right now, coherence measurement hard coded in
