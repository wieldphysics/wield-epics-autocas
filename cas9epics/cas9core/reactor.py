...
class Reactor(object):
                ...
                if self._pqueue:
                    ...
                    if ntime <= mtime:
                        ...
                        try:
                            ...
                            try:
                                self._queue_lock.release()
                                item = self._task_queue.get(True, min(ntime - mtime, self.max_wait_s))
                            except queue.Empty:
                                continue
                            finally:
                                self._queue_lock.acquire()
            ...
            if not qdata.has_run:
                ...
                if loop_settings is not None:
                    if loop_settings.period_s is not None:
                        ...
                        mtime_next = mtime_last + loop_settings.period_s - mtime_last % loop_settings.period_s
    ...
    def send_task_synchronous(self, func, *args, **kwargs):
        if self.check_living_in_reactor():
            ...
            while not ev_did_run.wait(timeout = .05):
                pass
            if len(retval) == 2:
                raise retval[1]
            else:
